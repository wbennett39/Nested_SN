...
def perform_convergence():
    N_ang_list = np.array([2,6,16,46,136, 406, 1216, 3646])
    cc_err = np.zeros((3, N_ang_list.size))
    gauss_err = np.zeros((3, N_ang_list.size))
    N_cells = 100
    psib, phib, cell_centersb, musb, tableaub, Jb = solve(N_cells = N_cells, N_ang = 4096, left_edge = 'source1', right_edge = 'source1', IC = 'cold', source = 'off',
    ...
    for iang, ang in enumerate(N_ang_list):
        ...
        gauss_err[2,iang] = RMSE(Jg[1], Jb[2])
        cc_err[0, iang] = RMSE(phicc, phib)
        cc_err[1, iang] = RMSE(Jb[0], Jcc[0])
        cc_err[2, iang] = RMSE(Jb[2], Jcc[2])
