...
def timedep_solve(tf = 1.0/10, dt = 500,  N_cells = 150, N_ang = 136, left_edge = 'vacuum', right_edge = 'vacuum', IC = 'pl', source = 'off',
          opacity_function = 'constant', wynn_epsilon = False, L = .2, source_strength = 1.0, sigma_a = 0.0, sigma_s = 10.0, sigma_t = 10.0, strength = [1.0,0.0]):
    ...
    for it in tqdm(range(1,dt)):
        ...
        psi, phi, cell_centers, mus, tableau, J, tableau_J, sigmas = solve(N_cells = N_cells, N_ang = N_ang, left_edge = left_edge, right_edge = right_edge, IC = 'cold', source = 'input',
        ...
        # assert 0
    plt.ion()
    bench = intg.plane_IC(tf * sigma_t, 100)
    plt.close()
    plt.plot(cell_centers, phi_list[-1]/sigma_t, label = f'phi + {N_ang}')
    
    plt.plot(bench[0]/sigma_t, bench[1] + bench[2], 'k-', label = 'benchmark')
    ...
    if wynn_epsilon == True:
        ...
        # plt.figure(4)
        bench_interpolated = interp1d(bench[0]/sigma_t, bench[1]+bench[2])
